...
function RunNikto($Niktourl)
{
	...
	if (document.getElementById('niktoRunOptionsList').value == 'terminal') {
				...
				private.notify('STeBB Notify', 'Nikto has started in terminal.');
	}
				...
				private.notify('STeBB Notify', notifyMsg );
	}
	else {
				...
				var terminal_path = currentWorkingDir+"/utilities/niktoSTeBB";
				...
				//alert (document.getElementById('urlbar').value);
				process.run(false, args, args.length);
				private.notify('STeBB Notify', 'Nikto has started. Result will be displayed when finished');
