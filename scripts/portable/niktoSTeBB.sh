...
type perl > /dev/null
		if [ $? -eq 0 ]; then
			...
			echo $1 >> niktoLog.txt
			cd  $BASEDIR/nikto/nikto
			perl nikto.pl $1 -F htm -output $BASEDIR/../../.stebb/nikto_results/niktoscan$CDATE.html

			$BASEDIR/../stebb $BASEDIR/../../.stebb/nikto_results/niktoscan$CDATE.html
		else
