...
type python > /dev/null
		if [ $? -eq 0 ]; then
			...
			cd $BASEDIR/wfuzz
			python wfuzz.py -c $1 2> $BASEDIR/../../.stebb/fuzz_results/wFuzzResult$CDATE.html

			$BASEDIR/../stebb $BASEDIR/../../.stebb/fuzz_results/wFuzzResult$CDATE.html
		else
			complain "Sorry,  python not installed."
		fi
